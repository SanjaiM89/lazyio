...
class YouTubeDownloader:
    ...
    async def get_video_info(self, url: str) -> Dict[str, Any]:
        ...
        if " - " in title:
            ...
            if len(parts) == 2:
                ...
                if (
                    potential_artist.lower() in artist.lower()
                    or artist.lower() in potential_artist.lower()
                ):
        ...
        def hook(d):
            ...
            if d["status"] == "downloading":
                ...
                if total > 0:
                    task.progress = (
                        downloaded / total
                    ) * 80  # Reserve 20% for conversion/upload
                ...
                if now - last_broadcast[0] > 2.0 or downloaded == total:
                    ...
                    print(
                        f"[YT] Downloading: {pct:.1f}% ({task.speed}, ETA: {task.eta})"
                    )
    ...
    async def download_video(
        ...
        if task_id and task_id in _download_tasks:
            ...
            task = DownloadTask(task_id=task_id, url=url, quality=quality)
        ...
        try:
            ...
            opts = {
                ...
                "progress_hooks": [
                    self._create_progress_hook(task, broadcast_callback)
                ],
                "postprocessors": [
                    {
            ...
            print(
                f"[YT] Looking for video files in {DOWNLOAD_DIR} with prefix {task_id}"
            )

            for filename in os.listdir(DOWNLOAD_DIR):
                if filename.startswith(task_id) and (
                    filename.endswith(".mp4") or filename.endswith(".mkv")
                ):
                    task.file_path = os.path.join(DOWNLOAD_DIR, filename)
                    task.file_size = os.path.getsize(task.file_path)
                    print(
                        f"[YT] Matched video file: {task.file_path} ({task.file_size} bytes)"
                    )
                    break
            ...
            )  # Reusing status for compression check
    ...
    async def download_audio(
        ...
        try:
            ...
            for filename in os.listdir(DOWNLOAD_DIR):
                ...
                if filename.startswith(task_id) and filename.endswith(
                    f".{expected_ext}"
                ):
                    task.file_path = os.path.join(DOWNLOAD_DIR, filename)
                    task.file_size = os.path.getsize(task.file_path)
                    print(
                        f"[YT] Matched file: {task.file_path} ({task.file_size} bytes)"
                    )
                    break
            ...
            is_youtube_block = (
                ...
                or "HTTP Error 403: Forbidden" in error_msg
            )
                ...
                try:
                    ...
                    if success and notifier:
                        ...
                        return await self.download_audio(
                            task_id, url, title, artist, quality, broadcast_callback
                        )
                    else:
                        if notifier:
