...
DOWNLOAD_DIR = os.path.join(
    os.path.dirname(__file__), "temp_uploads", "youtube_vidssave"
)
...
class VidsSaveDownloader:
    ...
    async def download_and_convert(
        self, url: str, task_id: Optional[str] = None, broadcast_callback=None
        ...
        try:
            ...
            print(
                f"[VidsSave] Selected: {best_video['quality']} ({best_video['size'] / 1024 / 1024:.1f} MB)"
            )
            ...
            async with self._conversion_sem:
                ...
                try:
                    ...
                    raise Exception(
                        f"Conversion failed (Exit {e.returncode}): {error_msg[-200:]}"
                    )
